...
(() => {
  ...
  function _pointInQuad(corners, lng, lat) {
    for (let i = 0; i < corners.length; i++) {
      ...
      if (Math.abs(cross) <= tolerance && lng >= Math.min(a[0], b[0]) - tolerance && lng <= Math.max(a[0], b[0]) + tolerance && lat >= Math.min(a[1], b[1]) - tolerance && lat <= Math.max(a[1], b[1]) + tolerance) {
        return true;
      }
    }
    let inside = false;
  ...
  function _vexcelClipPathToRect(path, width, height) {
    const segments = [];
    let current = [];
    ...
    for (let i = 1; i < path.length; i++) {
      const a = path[i - 1], b = path[i];
      if (!a || !b || !Number.isFinite(a[0]) || !Number.isFinite(a[1]) || !Number.isFinite(b[0]) || !Number.isFinite(b[1])) {
        flush();
        continue;
      }
      const dx = b[0] - a[0], dy = b[1] - a[1];
    ...
    const PerspectiveLayer = L.Layer.extend({
      ...
      _isPlannerLayer(layer) {
        ...
        if (typeof layer.getLatLng === "function") {
          const className = String(layer.options && layer.options.icon && layer.options.icon.options && layer.options.icon.options.className || "");
          return /(?:^|\s)circle(?:\s|$)/.test(className);
        }
        return false;
      },
      _isRouteMutation(record) {
        const hasRoute = (node) => !!(node && node.nodeType === 1 && (node.matches && node.matches(".route-polyline") || node.querySelector && node.querySelector(".route-polyline")));
      ...
      coversCurrentView() {
        ...
        return center[0] - halfX >= -tolerance && center[0] + halfX <= this._frame.w + tolerance && center[1] - halfY >= -tolerance && center[1] + halfY <= this._frame.h + tolerance;
      },
      getLoadedTileCount() {
        let count = 0;
      ...
      _eventPixel(event) {
        ...
        if (!Number.isFinite(point.x) || !Number.isFinite(point.y) || point.x < 0 || point.y < 0 || point.x > this._frame.w || point.y > this._frame.h) {
          return null;
        }
      ...
      _projectMapEvent(event, type) {
        ...
        if (target && target.closest && target.closest(
          ".leaflet-control,.leaflet-popup,.dw-vex-ctl,.leaflet-interactive,.leaflet-marker-icon,.leaflet-tooltip,.dw-vex-route-hit,.dw-vex-route-handle"
        )) return;
      ...
      _routeClick(event) {
        ...
        if (handle) {
          ...
          if (markerEntry && markerEntry.marker && typeof markerEntry.marker.fire === "function") {
            ...
            }, () => this._map && this._map.hasLayer(marker) && this._routeMarkers.some((entry) => entry.marker === marker));
          }
          return;
        }
      ...
      _activateRouteHit(hit, event) {
        ...
        }, () => this._map && this._map.hasLayer(source.polyline) && this._routeSources.some((entry) => entry.polyline === source.polyline));
      },
      ...
      _routePointerDown(event) {
        ...
        if (markerEntry.marker.dragging && typeof markerEntry.marker.dragging.enabled === "function" && !markerEntry.marker.dragging.enabled()) return;
        event.preventDefault();
        event.stopPropagation();
      ...
      _update() {
        ...
        for (const tile of tiles) {
          ...
          if (record && record.failed && currentBase && record.lastTileBase && currentBase !== record.lastTileBase) {
            record.tries = 0;
            record.retryAt = 0;
          }
          if (record && record.failed && record.tries < 3 && Date.now() >= record.retryAt) {
            record.failed = false;
      ...
      _requestRoute() {
        ...
        const rawModel = options.getRouteModel ? options.getRouteModel() : {
          paths: options.getRoutePaths().map((points) => ({ points })),
          markers: []
        };
      ...
      _drawRoute(paths, markers, interactive = true) {
        ...
        for (const entry of paths) {
          ...
          const sourceWeight = Number.isFinite(Number(options2.weight)) ? Number(options2.weight) : 8;
          const sourceOpacity = Number.isFinite(Number(options2.opacity)) ? Number(options2.opacity) : 0.4;
        ...
        if (interactive) for (let markerIdx = 0; markerIdx < (markers || []).length; markerIdx++) {
          ...
          if (sourceMarker && sourceMarker.dragging && typeof sourceMarker.dragging.enabled === "function" && !sourceMarker.dragging.enabled()) {
      ...
      _loadTile(tile) {
        ...
        const tileBase = options.tileBase ? options.tileBase(this._frame) : this._frame.tileBase;
        ...
        const url = tileBase + "&downsample=" + tile.downsample + "&tile-x=" + tile.x + "&tile-y=" + tile.y;
    ...
    _ensureQueryAuth((token, session) => {
      ...
      gmJsonGet(
        ...
        (err, data, raw) => {
          ...
          const seen = /* @__PURE__ */ new Set(), caps = [];
  ...
  function _dwGetRouteModel() {
    try {
      ...
      const paths = [], markers = [], seenMarkers = /* @__PURE__ */ new Set();
          ...
          try {
            const visit = (part) => {
              ...
              if (!part.some(Array.isArray)) {
                const points = part.filter((point) => point && Number.isFinite(point.lat) && Number.isFinite(point.lng)).map((point) => [point.lng, point.lat]);
  ...
  function createVexcelControl() {
    ...
    const obliqueCollection = () => {
      ...
      return ctl.capIdx === 0 && ctl.obModel.captures[0] ? ctl.obModel.captures[0].collection : curCollection();
    };
    const cellFor = (dir) => {
    ...
    const updateIrBtn = () => {
      if (!irBtn) return;
      const available = !ctl.obliqueActive || ctl.frameBand === "irg" || !!(ctl.obModel && cellFor(ctl.dir) && cellFor(ctl.dir).irg);
    ...
    const load = () => {
      if (!ctl.obModel) {
        hideOblique("No Vexcel oblique here — recentre over a flown area.");
        return;
      }
      if (!cellFor(ctl.dir)) {
      ...
      if (!img) {
        hideOblique("No " + _dirLabel(ctl.dir) + " photo for " + (curCollection() || "this date") + " here.");
        return;
      }
    ...
    ctl.remove = () => {
      ...
      if (ctl._modeObserver) {
        ctl._modeObserver.disconnect();
        ctl._modeObserver = null;
      }
      clearTimeout(refreshTimer);
      clearTimeout(frameTimer);
      clearTimeout(modelTimer);
  ...
  var VexcelLayerProvider = class extends LayerProvider {
    create() {
      ...
      const tileUrl = (z, x, y) => {
        ...
        let u = CFG.VEXCEL_API_BASE + "/v2/ortho/tile?zoom=" + z + "&tile-x=" + x + "&tile-y=" + y + "&interpolation=true&layer=urban,wide-area&session=" + encodeURIComponent(sess) + "&token=" + encodeURIComponent(tok);
        if (layer._dwVexColl) {
          u += "&order-by=" + encodeURIComponent(layer._dwVexColl) + ",collection-last-capture-date-desc";
        }
  ...
  // src/providers/fog-of-world.js
  var FILENAME_MASK1 = "olhwjsktri";
  var FILENAME_MASK2 = "eizxdwknmo";
  var FOW_MAP_WIDTH = 512;
  var FOW_TILE_WIDTH = 8192;
  var FOW_BLOCKS = 128;
  var FOW_BLOCK_WIDTH = 64;
  var FOW_HEADER_SIZE = FOW_BLOCKS * FOW_BLOCKS * 2;
  var FOW_BITMAP_SIZE = 512;
  var FOW_BLOCK_SIZE = 515;
  var FOW_NATIVE_ZOOM = 22;
  ...
  function md5ShortAscii(value) {
    ...
    for (let i = 0; i < 64; i++) {
      ...
      const sum = a + f + (Math.floor(Math.abs(Math.sin(i + 1)) * 4294967296) | 0) + words[g] | 0;
  ...
  function _fowDecodeFilename(filename) {
  ...
  function _fowParseInflated(filename, bytes) {
  ...
  function _fowVisited(bitmap, x, y) {
  ...
  function canvasPng(canvas) {
    return new Promise((resolve, reject) => {
      canvas.toBlob((blob) => {
        if (!blob) {
          reject(new Error("Fog of World tile encoding failed"));
          return;
        }
  ...
  var DropboxFogSource = class {
    ...
    async _downloadChunk(id) {
      ...
      if (!compressed) {
        this.missingCount++;
        this._setStatus("wait", "no chunk " + filename + " (loaded " + this.loadedCount + ", absent " + this.missingCount + ")");
        return null;
      }
      const chunk = _fowParseInflated(filename, await inflateZlib(compressed));
      this.loadedCount++;
      this._setStatus("ok", "loaded " + filename + " (" + chunk.blocks.size + " blocks; " + this.loadedCount + " chunks ready)");
      return chunk;
    }
  ...
  var FogOfWorldLayerProvider = class extends LayerProvider {
    create() {
  ...
  function fetchHistMapSheets(map, latlng, cb) {
    arcgisIdentify(map, latlng, {
      ...
      const seen = /* @__PURE__ */ new Set();
      const sheets = results.map((r) => _histMapSheet(r.attributes || {})).filter((m) => {
        if (!m.link || seen.has(m.link)) return false;
        seen.add(m.link);
        const s = parseFloat(m.scale);
        return !Number.isFinite(s) || s <= _HIST_MAX_SCALE;
      });
      sheets.sort((a, b) => (parseFloat(a.scale) || 9e9) - (parseFloat(b.scale) || 9e9) || Number(b.year || 0) - Number(a.year || 0));
      cb(sheets);
    });
  }
